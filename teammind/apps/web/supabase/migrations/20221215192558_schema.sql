/*
 * -------------------------------------------------------
 * Supabase TeamMind SaaS
 * This is the schema for the Supabase TeamMind SaaS.
 ...
 * -------------------------------------------------------
 */
-- Create a private TeamMind App schema
create schema if not exists tm;

create extension if not exists "unaccent" schema tm;
...
/*
* Permissions
- We create the permissions for the Supabase TeamMind App. These permissions are used to manage the permissions for the roles
...
/*
* Subscription Status
- We create the subscription status for the Supabase TeamMind App. These statuses are used to manage the status of the subscriptions
...
/*
Payment Status
- We create the payment status for the Supabase TeamMind App. These statuses are used to manage the status of the payments
*/
create type public.payment_status as ENUM('pending', 'succeeded', 'failed');

/*
* Billing Provider
- We create the billing provider for the Supabase TeamMind App. These providers are used to manage the billing provider for the accounts
- The providers are 'stripe', 'lemon-squeezy', 'paddle' and 'manual'.
- You can add more providers as needed.
*/
create type public.billing_provider as ENUM('stripe', 'lemon-squeezy', 'paddle', 'manual');

/*
* Subscription Item Type
- We create the subscription item type for the Supabase TeamMind App. These types are used to manage the type of the subscription items
...
/*
* Invitation Type
- We create the invitation type for the Supabase TeamMind App. These types are used to manage the type of the invitation
*/
 ...
 * We create the configuration for the Supabase TeamMind App to enable or disable features
 * -------------------------------------------------------
 */
create table if not exists
  public.config (
    ...
    billing_provider public.billing_provider default 'manual' not null
  );

comment on table public.config is 'Configuration for the Supabase TeamMind App.';
 ...
 * -------------------------------------------------------
 * Section: Accounts
 * We create the schema for the accounts. Accounts are the top level entity in the Supabase TeamMind App. They can be team or personal accounts.
 * -------------------------------------------------------
 */
-- Accounts table
create table if not exists
...
comment on table public.accounts is 'Accounts are the top level entity in the Supabase TeamMind App. They can be team or personal accounts.';
...
-- Function "tm.protect_account_fields"
-- Function to protect account fields from being updated
create
or replace function tm.protect_account_fields () returns trigger as $$
begin
...
execute function tm.protect_account_fields ();
...
-- Function "tm.add_current_user_to_new_account"
-- Trigger to add the current user to a new account as the primary owner
create
or replace function tm.add_current_user_to_new_account () returns trigger language plpgsql security definer
set
  search_path = '' as $$
begin
...
execute function tm.add_current_user_to_new_account ();

-- create a trigger to update the account email when the primary owner email is updated
create
or replace function tm.handle_update_user_email () returns trigger language plpgsql security definer
set
  search_path = '' as $$
begin
    update
...
execute procedure tm.handle_update_user_email ();
...
create table if not exists
  public.accounts_memberships (
    ...
    -- Add constraint to ensure user can only be in one non-personal account
    CONSTRAINT single_team_membership UNIQUE (user_id)
  );
...
-- Function "tm.prevent_account_owner_membership_delete"
-- Trigger to prevent a primary owner from being removed from an account
create
or replace function tm.prevent_account_owner_membership_delete () returns trigger
set
  search_path = '' as $$
begin
    if exists(
...
execute function tm.prevent_account_owner_membership_delete ();

-- Function "tm.prevent_memberships_update"
-- Trigger to prevent updates to account memberships with the exception of the account_role
create
or replace function tm.prevent_memberships_update () returns trigger
set
  search_path = '' as $$
begin
...
execute function tm.prevent_memberships_update ();
...
execute procedure tm.check_team_account ();
...
begin
    -- check if user is already a member of any team account
    -- fix the ambiguous column reference by explicitly referencing the parameter
    if exists(
        select
            1
        from
        ...
        where
            am.user_id = accept_invitation.user_id
            and a.is_personal_account = false) then
        raise exception 'User is already a member of a team account';
    end if;
...
-- Function "tm.update_notification_dismissed_status"
-- Make sure the only updatable field is the dismissed status and nothing else
create
or replace function tm.update_notification_dismissed_status () returns trigger
set
  search_path to '' as $$
begin
...
execute procedure tm.update_notification_dismissed_status ();
...
create
or replace function tm.slugify ("value" text) returns text as $$
    -- removes accents (diacritic signs) from a given string --
    with "unaccented" as(
        select
            tm.unaccent("value") as "value"
),
...
grant
execute on function tm.slugify (text) to service_role,
authenticated;

-- Function "tm.set_slug_from_account_name"
-- Set the slug from the account name and increment if the slug exists
create
or replace function tm.set_slug_from_account_name () returns trigger language plpgsql security definer
set
  search_path = '' as $$
declare
    sql_string varchar;
...
begin
    ...
    while tmp_row_count > 0 loop
        if increment > 0 then
            tmp_slug = tm.slugify(new.name || ' ' || increment::varchar);

        else
            tmp_slug = tm.slugify(new.name);
...
execute procedure tm.set_slug_from_account_name ();
...
-- Function "tm.setup_new_user"
-- Setup a new user account after user creation
create
or replace function tm.setup_new_user () returns trigger language plpgsql security definer
set
  search_path = '' as $$
declare
    user_name text;
...
execute procedure tm.setup_new_user ();
...
begin
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'You are already a member of a team account';
    END IF;
...
create
or replace function tm.get_storage_filename_as_uuid (name text) returns uuid
set
  search_path = '' as $$
begin
...
grant
execute on function tm.get_storage_filename_as_uuid (text) to authenticated,
service_role;

-- RLS policies for storage
create policy account_image on storage.objects for all using (
  bucket_id = 'account_image'
  and tm.get_storage_filename_as_uuid (name) = (
    select
      auth.uid ()
  )
  or public.has_role_on_account (tm.get_storage_filename_as_uuid (name))
)
with
  check (
    bucket_id = 'account_image'
    ...
    or public.has_permission (
      auth.uid (),
      tm.get_storage_filename_as_uuid (name),
      'settings.manage'
    ))
  );