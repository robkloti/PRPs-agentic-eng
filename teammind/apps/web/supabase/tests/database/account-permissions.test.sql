...
select row_eq(
  ...
  auth.uid(), teammind.get_account_id_by_slug('test'), 'members.manage'::app_permissions) $$,
    row(true::boolean),
...
select row_eq(
  $$ select public.has_permission(
  auth.uid(), teammind.get_account_id_by_slug('test'), 'billing.manage'::app_permissions) $$,
    row(true::boolean),
...
update public.accounts_memberships
    ...
    where account_id = teammind.get_account_id_by_slug('test')
