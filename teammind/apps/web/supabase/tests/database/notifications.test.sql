...
-- service role can insert into notifications
select lives_ok(
    $$ insert into public.notifications(account_id, body) values (teammind.get_account_id_by_slug('teammind'), 'test'); $$,
    'service role can insert into notifications'
);
...
select row_eq(
    $$ select account_id, body from public.notifications where account_id = teammind.get_account_id_by_slug('teammind'); $$,
    row (teammind.get_account_id_by_slug('teammind'), 'test'::varchar),
    'user can read their team notifications'
);
