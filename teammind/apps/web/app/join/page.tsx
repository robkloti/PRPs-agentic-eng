...
import { AcceptInvitationContainer } from '@tm/team-accounts/components';
...
async function JoinTeamAccountPage({ searchParams }: Context) {
  const token = (await searchParams).invite_token;
  ...
  if (account) {
    const { getLogger } = await import('@tm/shared/logger');
