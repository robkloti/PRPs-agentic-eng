...
import { Search, FileText, BookA, Sparkles, ArrowRight, ChevronRight, ChevronDown, Loader2 } from 'lucide-react';
import { Tweet } from 'react-tweet';
import React, { useState, useEffect, useRef } from 'react';
import { Badge } from "@tm/ui/badge";
import { Card, CardContent } from "@tm/ui/card";
import { Progress } from "@tm/ui/progress";
import { Tabs, TabsList, TabsTrigger } from "@tm/ui/tabs";
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogTrigger } from "@tm/ui/dialog";
import { Drawer, DrawerContent, DrawerHeader, DrawerTitle, DrawerTrigger } from "@tm/ui/drawer";
import { Accordion, AccordionContent, AccordionItem, AccordionTrigger } from "@tm/ui/accordion";
import { XLogo } from '@phosphor-icons/react';
import { cn, useMediaQuery } from "@tm/ui/utils";
...
// Define the types for the tool
// We need to modify this to accept both direct arguments and the tool's result structure
interface ReasonSearchArgs {
    topic?: string;
    depth?: 'basic' | 'advanced';
}

// This represents the update object that comes through argsText
interface ReasonSearchUpdateDelta {
    id: string;
    type: 'plan' | 'web' | 'academic' | 'analysis' | 'progress' | 'x';
    status: 'running' | 'completed';
    title?: string;
...
export const ReasonSearchToolUI = makeAssistantToolUI<ReasonSearchArgs, any>({
    toolName: 'reason_search', // Make sure this matches the name used in the server-side
    render: ({ args, status, argsText, result }) => {
        const [updates, setUpdates] = useState<StreamUpdate[]>([]);
        const [isCollapsed, setIsCollapsed] = useState(false);
        const [selectedTab, setSelectedTab] = useState("web");

        // Process argsText as it changes - this contains the streaming updates
        useEffect(() => {
            if (!argsText) return;

            try {
                // Parse the argsText which contains our update
                const updateData = JSON.parse(argsText);

                // Skip if missing required fields
                if (!updateData || !updateData.id || !updateData.type || !updateData.status) return;

                // Convert to StreamUpdate
                const newUpdate: StreamUpdate = {
                    ...
                    gaps: updateData.gaps,
                    recommendations: updateData.recommendations,
                    uncertainties: updateData.uncertainties
                };

                // Update state, handling overwrites
                setUpdates(prevUpdates => {
                    // Create a map of existing updates
                    const updatesMap = new Map<string, StreamUpdate>();
                    prevUpdates.forEach(update => {
                        updatesMap.set(update.id, update);
                    });

                    // Handle the new update
                    if (newUpdate.overwrite || !updatesMap.has(newUpdate.id)) {
                        updatesMap.set(newUpdate.id, newUpdate);
                    } else {
                        const existing = updatesMap.get(newUpdate.id)!;
                        // Only overwrite running updates with completed ones
                        if (newUpdate.status === 'completed' && existing.status !== 'completed') {
                            updatesMap.set(newUpdate.id, newUpdate);
                        }
                    }
                ...
                console.error("Error parsing argsText:", e);
            }
        }, [argsText]);

        // Also process direct args for compatibility (initial parameters)
        useEffect(() => {
            // Check if args contains a direct update (from a prior implementation)
            if (args && typeof args === 'object' && 'id' in args && 'type' in args && 'status' in args) {
                const updateData = args as unknown as ReasonSearchUpdateDelta;
        ...
        // Process final result for completeness
        useEffect(() => {
            if (result && result.plan) {
                // When final result arrives, make sure we mark research as complete
                setUpdates(prevUpdates => {
                    const updatesMap = new Map<string, StreamUpdate>();
                    prevUpdates.forEach(update => {
                        updatesMap.set(update.id, update);
                    ...
                    // Add a completion update if it doesn't exist
                    if (!updatesMap.has('research-progress')) {
                        updatesMap.set('research-progress', {
                            ...
                            isComplete: true,
                            completedSteps: result.results?.length || 0,
        ...
        // Check if we're still loading with no updates
        if (status.type === 'running' && updates.length === 0) {
            return (
                <Card className="w-full shadow-none hover:shadow-none mb-4">
                    <div className="flex items-center px-4 sm:px-6 py-3 sm:py-4 rounded-xl">
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                            ...
                            <Progress
                                value={10}
                                className="h-1 w-24 sm:w-32 animate-pulse"
                            />
                        </div>
                    </div>
                </Card>
