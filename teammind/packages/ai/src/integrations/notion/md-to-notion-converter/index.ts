...
import {
  BlocksOptions,
  RichTextOptions,
  markdownToBlocks as internalMarkdownToBlocks,
  parseRichText,
...
export function markdownToBlocks(
  ...
  return internalMarkdownToBlocks(body, options);
}
...
export function markdownToRichText(
  ...
  const root = unified().use(markdown).use(gfm).use(remarkMath).parse(text);
  return parseRichText(root as unknown as md.Root, options);
}

// Re-export the options types for easier access
export type { BlocksOptions, RichTextOptions };
