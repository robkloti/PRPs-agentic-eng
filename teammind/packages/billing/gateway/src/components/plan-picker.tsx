...
import { formatCurrency } from '@tm/shared/utils';
...
import { Label } from '@tm/ui/label';
import {
  RadioGroup,
  RadioGroupItem,
  RadioGroupItemLabel,
} from '@tm/ui/radio-group';
import { Separator } from '@tm/ui/separator';
...
export function PlanPicker(
  ...
  const handleManualBillingRedirect = () => {
    if (props.accountSlug) {
      const planId = form.getValues('planId');
      const productId = form.getValues('productId');
      // Navigate to support page with billing issue type and plan info
      window.location.href = `/home/${props.accountSlug}/settings/support?issueType=billing&planId=${planId}&productId=${productId}`;
    }
  };
        ...
        >
          ...
          <div>
            <Button
              ...
              onClick={
                props.isManualBilling ? handleManualBillingRedirect : undefined
              }
              type={props.isManualBilling ? 'button' : 'submit'}
            >
              {props.pending ? (
                ...
                <>
                  <If
                    condition={props.isManualBilling}
                    fallback={
                      <If
                        condition={
