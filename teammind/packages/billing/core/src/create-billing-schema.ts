...
// Define a schema for usage limits with corrected naming
export const UsageLimitsSchema = z.object({
  chatCharactersPerFiveHours: z
    .number({
      description:
        'Maximum number of characters allowed in chat messages within a 5-hour window',
    })
    .nonnegative(),
  meetingHoursPerMonth: z
    .number({
      description: 'Maximum number of meeting hours allowed per month',
    })
    .nonnegative(),
});
...
const ProductSchema = z
  .object({
    ...
    highlighted: z
      ...
      .optional(),
    // Add usage limits to the product schema with corrected property name
...
/**
 * Get usage limits for a specific product
 * @param config Billing configuration
 * @param productId Product ID to get limits for
 * @returns Usage limits or default limits if not defined
 */
export function getProductUsageLimits(
  config: z.infer<typeof BillingSchema>,
  productId: string,
): UsageLimits {
  const defaultLimits: UsageLimits = {
    chatCharactersPerFiveHours: 1000,
    meetingHoursPerMonth: 0,
  };

  for (const product of config.products) {
    if (product.id === productId) {
      return product.usageLimits ?? defaultLimits;
    }
  }
