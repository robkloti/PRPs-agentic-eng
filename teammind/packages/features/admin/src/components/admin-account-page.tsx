...
import { ProfileAvatar } from '@tm/ui/profile-avatar';
...
import { AdminSubscriptionManager } from './admin-subscription-manager';
...
async function PersonalAccountPage(props: { account: Account }) {
  ...
  const subscription = await getSubscription(props.account.id);
...
async function TeamAccountPage(props: {
  ...
  const isPendingOwnerTransfer =
    typeof props.account.public_data === 'object' &&
    props.account.public_data !== null
      ? (props.account.public_data as { pending_owner_email?: string })
          .pending_owner_email
      : undefined;
    ...
    <div className={'flex flex-col space-y-4'}>
      <div className={'flex justify-between'}>
        <div className={'flex items-center space-x-4'}>
          ...
          {isPendingOwnerTransfer && (
            <Badge variant={'warning'}>
              Pending Owner Transfer to{' '}
              {typeof props.account.public_data === 'object' &&
              props.account.public_data !== null
                ? (
                    props.account.public_data as {
...
async function getSubscription(accountId: string) {
  const client = getSupabaseServerClient({
    admin: true,
  });
